...
make descriptive statistics for amp_score in analysis.Rmd, just as you did for the self reported evaluations. As the response to this assignment, reply with the mean AMP score in women, taken from your AMP descriptive statistics table. 
...
bind_rows(dat_mean_amp,
  ...
  select(subset_amp = group, Mean, SD) |> # select variables of interest, and rename one 
  mutate(subset_amp = stringr::str_to_sentence(subset_amp)) |> # Change the case of the Subset variable so that it prints nicely
  mutate_if(is.numeric, round_half_up, digits = 2) |>
  kable() |>
