...
data_amp_score <- data_amp_raw |> 
  filter(blockcode != "practice",
         trialcode != "instructions") |> 
  select(subject, trialcode, blockcode, correct, latency) |> 
  ...
  summarize(AMP_score = sum(evaluation == "true")/72,
             n_values = n()) |> #first sanity check
  mutate(is_AMP_score_a_proportion = if_else(AMP_score < 1 & AMP_score > 0, TRUE, FALSE)) #second
